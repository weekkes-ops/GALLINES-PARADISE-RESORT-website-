...
export const SportsWellnessSection: React.FC<SportsWellnessSectionProps> = ({ onOpenBooking }) => {
  const [activeTab, setActiveTab] = useState<'arena' | 'gym'>('arena');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-14">
          ...
          <h2 className="font-display text-3xl sm:text-4xl md:text-5xl font-bold text-[#2d2d2a] tracking-tight">
            Premier <span className="emerald-gradient-text">Sports Complex</span> & Fitness Gym
          </h2>
          <p className="text-[#686762] text-sm sm:text-base leading-relaxed">
            Elevate your wellness routine. Gallines Paradise is the only luxury resort in the region boasting both an all-weather fenced sports arena and a dedicated indoor fitness cardio suite.
          </p>

          {/* Toggle Tab */}
          <div className="inline-flex p-1.5 rounded-2xl bg-[#ffffff] border border-[#d8d4c7] shadow-sm mt-4">
            <button
              type="button"
              onClick={() => setActiveTab('arena')}
              className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs sm:text-sm font-bold uppercase tracking-wider transition-all cursor-pointer ${
                activeTab === 'arena'
